...
let activeElement = null; // reference to current HTML element where text is being appended
...
function handleLineStartMarker(token, index) {
    // only check at the start of a line and only in normal text mode
    if (mode !== 'text' || !isLineStart)
        return index;
...
function handleStars(count) {
    while (count > 0) {
        // double star => bold
        if (count >= 2) {
            if ((activeElement === null || activeElement === void 0 ? void 0 : activeElement.tagName) === 'STRONG') {
                activeElement = null;
            }
            else {
                createBold();
            ...
            // single star => italic
            if ((activeElement === null || activeElement === void 0 ? void 0 : activeElement.tagName) === 'EM') {
                activeElement = null;
            }
            else {
                createItalic();
